...
| filter_type | 字符串 | 否| Repositories type，枚举值：private, public, secret|
...
| status | 字符串 | 否| 仓库状态。Repository status，枚举值：active, archived|
| order_by | 字符串 | 否| Order field，枚举值：created_at, last_updated_at, stars, slug_path, forks|
| desc | 布尔值 | 否| Ordering|
| descendant | 字符串 | 否| 查全部/查询直接属于当前组织的仓库/查询子组织的仓库。Get all/Get repos belong to current org or sub-organization，枚举值：all, sub, grand|
