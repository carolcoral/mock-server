...
| page | 整数 | 否| 页码|
| page_size | 整数 | 否| 每页数量|
| registry_type | 字符串 | 否| 制品仓库类型，枚举值：npm, maven, ohpm|
| filter_type | 字符串 | 否| 制品仓库可见性类型，枚举值：private, public|
| order_by | 字符串 | 否| 排序类型，默认created_at，枚举值：created_at, name|
| desc | 布尔值 | 否| 排序顺序|
| descendant | 字符串 | 否| 查全部/查询直接属于当前组织的仓库/查询子组织的制品仓库，枚举值：all, sub, grand|
| search | 字符串 | 否| 搜索关键字|
## 响应信息
