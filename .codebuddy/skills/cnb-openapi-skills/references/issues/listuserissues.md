...
## 原始 Swagger
https://api.cnb.cool/#/operations/ListUserIssues
...
**请求地址：** ${CNB_API_ENDPOINT}/user/issues
...
| filter | 字符串 | 否| Issue筛选类型。可选值：`created`（我创建的）、`assigned`（分配给我的）|
...
| order_by | 字符串 | 否| Issue排序字段，-前缀表示倒序。可选值：`created_at`,`-created_at`,`-updated_at`|
## 响应信息


**响应类型：** 数组[api.UserIssue]
