# ListIssueActivitiesLatest

## 原始 Swagger
https://api.cnb.cool/#/operations/ListIssueActivitiesLatest
...
**请求地址：** ${CNB_API_ENDPOINT}/{repo}/-/issues/{number}/activities/latest/{id}
...
| id | 字符串 | 是 | Activity ID，查询此 ID 之后的动态。|
## 响应信息
...
curl -w '%header{traceparent}' -X GET \
  "${CNB_API_ENDPOINT}/{repo}/-/issues/{number}/activities/latest/{id}" \
