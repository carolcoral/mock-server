...
## 原始 Swagger
https://api.cnb.cool/#/operations/PostPullReviewers

## 接口描述
添加评审人到指定的合并请求。Adds up to reviewers to a pull request. Users already added as reviewers are not replaced.
## 接口权限
repo-pr:rw
## 请求信息
...
**请求地址：** ${CNB_API_ENDPOINT}/{repo}/-/pulls/{number}/reviewers
...
```json
{
  "reviewers": ["string"] // 合并请求的评审人用户列表，最多支持8个。
}
```
## 响应信息
...
curl -w '%header{traceparent}' -X POST \
  "${CNB_API_ENDPOINT}/{repo}/-/pulls/{number}/reviewers" \
